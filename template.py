...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<43} | {'GPT-4o Response':<43} | {'Mini Response':<43} | {'4o Latency':<10} | {'Mini Latency':<12}"
    separator = "-" * len(header)
    lines = [header, separator]

    for r in results:
        p = truncate(r.get("prompt", ""))
        g4o = truncate(r.get("gpt4o_response", ""))
        mini = truncate(r.get("mini_response", ""))
        lat_4o = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        lat_mini = f"{r.get('mini_latency', 0.0):.2f}s"
        lines.append(f"{p:<43} | {g4o:<43} | {mini:<43} | {lat_4o:<10} | {lat_mini:<12}")
